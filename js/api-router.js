...
const API_CONFIG = {
    // Authenticated Keys
    HUGGINGFACE_KEY: "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",
...
async function scanMathImage(imageFile) {
    // Note: Using a standard image-to-text model endpoint. 
    ...
    try {
        ...
        const response = await fetch(API_URL, {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${API_CONFIG.HUGGINGFACE_KEY}`,
...
async function fetchNasaAsteroidData() {
    // Gets data for asteroids approaching today
    ...
    try {
        ...
        // Extract the first asteroid of the day as an example
        ...
        if (asteroids && asteroids.length > 0) {
            return {
                ...
                estimated_mass_max_kg: asteroids[0].estimated_diameter.meters.estimated_diameter_max * 3000 // Rough mass estimate based on volume
            };
        }
        return null;
    } catch (error) {
...
async function fetchMaterialProperties(materialFormula) {
    ...
    try {
        ...
        return data.data[0] || null; // Returns bulk modulus, density, etc.
    } catch (error) {
...
async function fetchProteinData(proteinName) {
    ...
    try {
        ...
        const data = await response.json();
        return data[0]; // Returns complete sequence and function mapping
    } catch (error) {

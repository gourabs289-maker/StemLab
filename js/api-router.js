...
/* --------------------------------------------------------------------------
   3. ENGINEERING: THE MATERIALS PROJECT (PROXY BYPASS FOR WEB & EXPO)
   -------------------------------------------------------------------------- */
async function fetchMaterialProperties(materialFormula) {
    // The original strict API URL
    const targetUrl = `https://api.materialsproject.org/materials/summary/?formula=${materialFormula}`;
    
    // Wrapping it in a free public proxy to bypass web security blocks
    ...
    try {
        ...
        if (!response.ok) throw new Error("API or Proxy Blocked");
        const data = await response.json();

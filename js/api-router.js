...
async function fetchMaterialProperties(query) {
    ...
    const mpUrl = `https://api.materialsproject.org/materials/summary/?formula=${targetSymbol}`;
    ...
    try {
        ...
        console.warn("Materials Project blocked by web browser CORS. Termux required for physical stats."); 
    }

    // If both engines fail to find anything, return null to trigger the UI error
...
async function fetchParticleData(query) {
    ...
    // The 17 Fundamental Particles Truth Matrix
    const standardModel = {
